...
const browser = engine === 'webkit' ? await webkit.launch() : await chromium.launch({ args: ['--enable-unsafe-swiftshader', '--use-angle=swiftshader'] });
...
async function measure(url) {
  ...
  for (const id of ids) {
    ...
    const syncMs = await page.evaluate(id => {
      const el = document.getElementById(id), min = +el.min || 0, max = +el.max || 100, t = [];
      for (let i = 0; i < 40; i++) {
        el.value = String(i % 2 ? min + (max - min) * 0.6 : min + (max - min) * 0.4);
        const t0 = performance.now(); el.dispatchEvent(new Event('input', { bubbles: true })); t.push(performance.now() - t0);
      }
    ...
    out[id] = { dispatchMs, dragMs, syncMs };
  }
...
const lines = [`[${engine}] builds: ${urls.join(' vs ')}`];
...
for (const r of rows) lines.push(`[${engine}] ${r.id}: sync ${r.a.syncMs?.toFixed(3)} -> ${r.b.syncMs?.toFixed(3)} ms | input ${r.a.dispatchMs?.toFixed(1)} -> ${r.b.dispatchMs?.toFixed(1)} ms (x${r.ratio.toFixed(2)}) | drag ${r.a.dragMs?.toFixed(1) ?? '-'} -> ${r.b.dragMs?.toFixed(1) ?? '-'} ms/step (x${r.dragRatio.toFixed(2)})`);
