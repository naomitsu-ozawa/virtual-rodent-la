 ...
 for(var iter:u32=0u;iter<4096u;iter=iter+1u){
  ...
  if(capT>=t-1e-7&&capT<=nextT+1e-7){
   if(u.sectionCap.x>0.5){
    let cp=u.camOrigin.xyz+dir*capT;let ctc=texCoord(cp);let capValue=huAt(ctc);let capIndex=segmentIndex(capValue);
    if(capIndex>=0){
     var capColor=mix(u.segments[u32(capIndex)*2u+1u].rgb,vec3<f32>(1.0),0.18);
     if(u.sectionCap.z>0.5){
      ...
      if(stripe<0.085){capColor*=0.38;}
     }
     let ca=clamp(u.sectionCap.y,0.0,1.0);let contribution=(1.0-acc.a)*ca;
     acc=vec4<f32>(acc.rgb+capColor*contribution,acc.a+contribution);
    }
   }
   capT=1e30;
  }
