...
import { MedicalVolumeRenderer, extractSourceThresholdRuns } from './medical-volume.js?v=20260925-build181';
import { unzip } from 'https://esm.sh/fflate@0.8.2';
const APP_VERSION='2026.09.25-181';const APP_BUILD='181';
...
function initIPadWorkspaceUi(){
 ...
 document.documentElement.classList.add('vrl-ipad-ui');
 shell.classList.add('ipad-mode-3d');

 const toolbar=document.createElement('div');
 toolbar.id='ipad-workspace-toolbar';toolbar.className='ipad-workspace-toolbar';
 toolbar.innerHTML='<button id="ipad-settings-toggle" class="ipad-toolbar-button" type="button" data-i18n="ipadSettings">設定</button><div class="ipad-view-modes" role="group" aria-label="View mode"><button type="button" class="is-active" data-ipad-view-mode="3d" data-i18n="ipadView3d">3D</button><button type="button" data-ipad-view-mode="2d" data-i18n="ipadView2d">2D</button><button type="button" data-ipad-view-mode="split" data-i18n="ipadViewSplit">分割</button></div><div class="ipad-mpr-tabs" role="tablist" aria-label="MPR plane"><button type="button" class="is-active" data-ipad-mpr="axial">Axial</button><button type="button" data-ipad-mpr="coronal">Coronal</button><button type="button" data-ipad-mpr="sagittal">Sagittal</button></div>';
 topbar.after(toolbar);

 const backdrop=document.createElement('button');backdrop.type='button';backdrop.className='ipad-sidebar-backdrop';backdrop.setAttribute('aria-label','Close settings');shell.appendChild(backdrop);
 const drawerHead=document.createElement('div');drawerHead.className='ipad-drawer-head';
 drawerHead.innerHTML='<strong data-i18n="ipadSettings">設定</strong><button id="ipad-sidebar-close" type="button" data-i18n="ipadClose">閉じる</button>';
 sidebar.insertBefore(drawerHead,sidebarScroll);
 ...
 let editPanel=null;
 if(editDetails){
  editPanel=document.createElement('section');editPanel.className='panel ipad-edit-drawer-panel';editPanel.dataset.ipadDrawerSection='edit';
 ...
 const closeDrawer=()=>{document.documentElement.classList.remove('ipad-sidebar-open');refreshLayout()};
 const openDrawer=()=>{document.documentElement.classList.add('ipad-sidebar-open')};
 ...
 toolbar.querySelector('#ipad-settings-toggle')?.addEventListener('click',openDrawer);
 drawerHead.querySelector('#ipad-sidebar-close')?.addEventListener('click',closeDrawer);
 ...
 setDrawerTab('display');setMprPlane('axial');setViewMode('3d');applyLanguage(currentLanguage);
}
