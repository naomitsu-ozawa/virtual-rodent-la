...
const APP_VERSION='2026.09.25-159';const APP_BUILD='159';
...
function gpuVolumeEditDescriptors(v=sourceVolume||volume){
 ...
 for(const key of SEGMENT_PRESET_ORDER){
  ...
  if(st.keepRuns){out[key]={mode:'keep',runs:st.excludeRuns?subtractRunArrays(st.keepRuns,st.excludeRuns,v.slices):st.keepRuns}}
  else if(st.excludeRuns){out[key]={mode:'exclude',runs:st.excludeRuns}}
 }
 return out;
}
...
function scheduleCutResultPreview(delay=180){
 ...
 if(threeRenderMode==='volume'){clearCutResultPreview();request3DRender();return}
