...
fn editAllows(seg:u32,tc0:vec3<f32>)->bool{
 ...
 let keepMask=editRows[1],dims=vec3<u32>(u32(u.dimsSlope.x),u32(u.dimsSlope.y),u32(u.dimsSlope.z));
 let tc=clamp(tc0,vec3<f32>(0.0),vec3<f32>(0.999999)),p=min(vec3<u32>(tc*vec3<f32>(dims)),dims-vec3<u32>(1u));
 let rowCount=dims.y*dims.z,base=2u+seg*(rowCount+1u),row=p.z*dims.y+p.y,start=editRows[base+row],finish=editRows[base+row+1u];
 var inside=false;
 for(var i=start;i<finish;i=i+1u){
  let packed=editIntervals[i],x0=packed&65535u,x1=packed>>16u;
...
fn segmentIndexAt(tc0:vec3<f32>)->i32{
 let tc=clamp(tc0,vec3<f32>(0.0),vec3<f32>(0.999999)),v=huAt(tc);
...
@fragment fn fs(@builtin(position) frag:vec4<f32>)->@location(0) vec4<f32>{
 ...
 for(var iter:u32=0u;iter<4096u;iter=iter+1u){
  ...
  if(canSample&&!capDrawn){
   let tc0=texCoord(p);let value=huAt(tc0);let idx=segmentIndexAt(tc0);
   if(idx!=lastIndex){
    if(idx>=0){
...
fn editAllows(seg:u32,tc0:vec3<f32>)->bool{
 ...
 let rowCount=dims.y*dims.z,base=2u+seg*(rowCount+1u),row=p.z*dims.y+p.y,start=editRows[base+row],finish=editRows[base+row+1u];
 var inside=false;
 for(var k=start;k<finish;k=k+1u){let packed=editIntervals[k],x0=packed&65535u,x1=packed>>16u;if(p.x<x0){break;}if(p.x<=x1){inside=true;break;}}
 let keep=(keepMask&(1u<<seg))!=0u;return select(!inside,inside,keep);
}
fn segmentIndexAt(tc0:vec3<f32>,preferred:i32)->i32{
 let tc=clamp(tc0,vec3<f32>(0.0),vec3<f32>(0.999999)),v=huAt(tc);
 if(preferred>=0){
  let s=u32(preferred),a=u.segments[s*2u];if(a.w>0.5&&v>=a.x&&v<=a.y&&editAllows(s,tc)){return preferred;}return -1;
 }
...
@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) gid:vec3<u32>){
 let i=gid.x,count=u32(pick[0]);if(i>=count){return;}
 let ib=4u+i*4u,ob=i*4u;result[ob]=0u;result[ob+1u]=0u;result[ob+2u]=0u;result[ob+3u]=0u;
...
export class MedicalVolumeRenderer{
 ...
 rebuildBindGroup(){
  if(!this.texture||!this.brickBuffer||!this.editRowsBuffer||!this.editIntervalsBuffer){this.bindGroup=null;return}
 ...
 setEditRuns(edits,segmentOrder,v){
  if(!v||!segmentOrder?.length){this.clearEditRuns();return}
 ...
 resetData(){this.setActive(false);this.texture?.destroy?.();this.brickBuffer?.destroy?.();this.texture=null;this.brickBuffer=null;this.bindGroup=null;this.seriesId=null;this.bricksReady=false;this.previewVolume=null;this.previewPlaneBuffers={coronal:null,sagittal:null};this.volume=null;this.clearEditRuns()}
 destroy(){this.resetData();this.uniformBuffer?.destroy?.();this.pickBuffer?.destroy?.();this.pickOutput?.destroy?.();this.editRowsBuffer?.destroy?.();this.editIntervalsBuffer?.destroy?.();this.mprUniformBuffer?.destroy?.();this.canvas.remove()}
}
