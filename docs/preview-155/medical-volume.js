...
@fragment fn fs(@builtin(position) frag:vec4<f32>)->@location(0) vec4<f32>{
 ...
 var t=max(bounds.x,0.0);var endT=bounds.y;let step=max(u.halfStep.w,0.00001);
 if(u.section.x>0.5){
 ...
 for(var iter:u32=0u;iter<4096u;iter=iter+1u){
  ...
  if(canSample){
   let value=huAt(texCoord(p));let idx=segmentIndex(value);
   if(idx!=lastIndex){
    if(idx>=0){
...
export class MedicalVolumeRenderer{
 ...
 render(camera,obj,segmentState,segmentOrder,mpr={}){
  ...
  const data=new Float32Array(80),put=(slot,a,b,c,d)=>{const i=slot*4;data[i]=a;data[i+1]=b;data[i+2]=c;data[i+3]=d};
