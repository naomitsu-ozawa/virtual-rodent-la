...
function volumeTexturePlan(v,maxTextureBytes=0,maxTextureDim=Infinity){
...
export class MedicalVolumeRenderer{
 ...
 support(v,{maxTextureBytes=0}={}){
  ...
  const lim=this.device.limits.maxTextureDimension3D,plan=volumeTexturePlan(v,maxTextureBytes,lim);
  if(plan.dims[0]>lim||plan.dims[1]>lim||plan.dims[2]>lim)return{ok:false,reason:'Volume exceeds maxTextureDimension3D '+lim};
  return{ok:true,plan};
 }
 async ensure(v,{prepareBricks=true,previewSide=0,maxTextureBytes=0}={}){
  const support=this.support(v,{maxTextureBytes});if(!support.ok)throw new Error(support.reason);
  const s=v.series,plan=support.plan,[tw,th,td]=plan.dims;
