  ...
  if(canSample&&!capDrawn){
   ...
   if(idx!=lastIndex){
    if(idx>=0){
     ...
     if(isCutPreview){col=vec3<f32>(1.0,0.16,0.055);alpha=max(alpha,0.94);}
     let lit=select(col*diffuse+vec3<f32>(spec),col*0.92+vec3<f32>(0.12,0.015,0.0),isCutPreview);
...
export class MedicalVolumeRenderer{
 ...
 rebuildBindGroup(){
  if(!this.texture||!this.brickBuffer||!this.editRowsBuffer||!this.editIntervalsBuffer||!this.previewRowsBuffer||!this.previewIntervalsBuffer){this.bindGroup=null;return}
 ...
 destroy(){this.resetData();this.uniformBuffer?.destroy?.();this.pickBuffer?.destroy?.();this.pickOutput?.destroy?.();this.editRowsBuffer?.destroy?.();this.editIntervalsBuffer?.destroy?.();this.previewRowsBuffer?.destroy?.();this.previewIntervalsBuffer?.destroy?.();this.mprUniformBuffer?.destroy?.();this.canvas.remove()}
}
