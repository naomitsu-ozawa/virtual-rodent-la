...
import { MedicalVolumeRenderer, extractSourceThresholdRuns } from './medical-volume.js?v=20260925-build178';
import { unzip } from 'https://esm.sh/fflate@0.8.2';
const APP_VERSION='2026.09.25-178';const APP_BUILD='178';
...
async function activateMedicalVolume(){
 ...
 const mobileBudget=mobileGpuVolumeBudget(),support=mv.support(target,{maxTextureBytes:mobileBudget});if(!support.ok){footer.textContent='GPU Volume: '+support.reason;updateRenderModeControl(target);return}
...
function mobileGpuVolumeBudget(){
 ...
 if(isIPadRuntime())return 192*1024*1024;
 return 0;
}
function shouldAutoPrepareResidentGpu(v){
 const mv=sceneState?.medicalVolume;if(!mv||!v?.sourceBacked)return false;
 const support=mv.support(v,{maxTextureBytes:mobileGpuVolumeBudget()});return !!support.ok;
}
...
async function prepareResidentGpuVolume(v){
 const mv=sceneState?.medicalVolume;if(!mv||!v?.sourceBacked)return false;
 const mobileBudget=mobileGpuVolumeBudget(),support=mv.support(v,{maxTextureBytes:mobileBudget});if(!support.ok)return false;
 const seriesId=v?.series?.id||null;residentGpuUploadSeriesId=seriesId;cancelSourceMprWarmup();
 try{
  const previewSide=mobileBudget?0:384;
  await mv.ensure(v,{prepareBricks:false,previewSide,maxTextureBytes:mobileBudget});
