...
import { MedicalVolumeRenderer, extractSourceThresholdRuns } from './medical-volume.js?v=20260925-build173';
import { unzip } from 'https://esm.sh/fflate@0.8.2';
const APP_VERSION='2026.09.25-173';const APP_BUILD='173';
...
async function start3D(){
 ...
 const setFastInteraction=active=>{
  const next=!!active;if(fastInteractionActive===next)return;fastInteractionActive=next;
  active3DPixelRatio=next?interactive3DPixelRatio:full3DPixelRatio;
  renderer.setPixelRatio(active3DPixelRatio);renderer.setSize(viewport.clientWidth,viewport.clientHeight,false);
  sceneState?.medicalVolume?.setInteractive?.(next);setHeavyOverlayInteraction(next);request3DRender();
 };
