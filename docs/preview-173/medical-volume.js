...
export class MedicalVolumeRenderer{
 constructor({device,host,rendererCanvas,onProgress,onStatus}){
  ...
  this.texture=null;this.bindGroup=null;this.seriesId=null;this.bricksReady=false;this.previewVolume=null;this.previewPlaneBuffers={coronal:null,sagittal:null};this.active=false;this.interactive=false;this.halfExtents=[1,1,1];this.step=0.002;this.calibration={slope:1,intercept:0,signedBias:0};this.volume=null;
  this.frameData=new Float32Array(84);this.tmpInv=new THREE.Matrix4();this.tmpOrigin=new THREE.Vector3();this.tmpQuat=new THREE.Quaternion();this.tmpRight=new THREE.Vector3();this.tmpUp=new THREE.Vector3();this.tmpForward=new THREE.Vector3();
 }
 support(v){
 ...
 setInteractive(active){
  const next=!!active;if(this.interactive===next)return;
  this.interactive=next;this.resize(true);
 }
 resize(force=false){
  const dpr=window.devicePixelRatio||1,ratio=this.interactive?Math.min(dpr,((navigator.maxTouchPoints||0)>0 ? 0.72 : 0.9)):Math.min(dpr,1.5),w=Math.max(1,Math.floor(this.host.clientWidth*ratio)),h=Math.max(1,Math.floor(this.host.clientHeight*ratio));
 ...
 render(camera,obj,segmentState,segmentOrder,mpr={}){
  ...
  put(4,this.halfExtents[0],this.halfExtents[1],this.halfExtents[2],this.interactive?this.step*1.65:this.step);
