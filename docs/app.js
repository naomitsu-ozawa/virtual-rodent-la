...
const APP_VERSION='2026.09.23-97';const APP_BUILD='97';
...
function cutFrameFromSurface(surface,camera){
 if(!surface?.hit?.point||!sceneState?.obj)return null;
 const obj=sceneState.obj,viewDir=new THREE.Vector3();camera.getWorldDirection(viewDir).normalize();
 ...
 return{plane:new THREE.Plane().setFromNormalAndCoplanarPoint(viewDir,surface.hit.point.clone()),ray:surface.ray,right:surface.right,up:surface.up,key:surface.key||null};
}
function cutPointerVoxel(event,canvas,camera,preferredKey=null,frame=null){
 if(!sceneState?.obj||!volume)return null;
 if(!frame)return surfaceSegmentPointerVoxel(event,canvas,camera,preferredKey);
 const rect=canvas.getBoundingClientRect(),mouse=new THREE.Vector2(((event.clientX-rect.left)/Math.max(rect.width,1))*2-1,-((event.clientY-rect.top)/Math.max(rect.height,1))*2+1),raycaster=new THREE.Raycaster();raycaster.setFromCamera(mouse,camera);
 const world=new THREE.Vector3();if(!raycaster.ray.intersectPlane(frame.plane,world))return null;
 ...
 const surface=surfaceSegmentPointerVoxel(event,canvas,camera,preferredKey),key=surface?.key||preferredKey||frame.key||null;
 return{x:(local.x/scale+px/2)/vx,y:(-local.y/scale+py/2)/vy,z:(local.z/scale+pz/2)/vz,ray:frame.ray,right:frame.right,up:frame.up,hit:surface?.hit||null,key,virtual:!surface};
}
