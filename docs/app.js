...
const APP_VERSION='2026.09.23-47';const APP_BUILD='47';
...
const analysisNavigateButton=$('#analysis-navigate'),analysisCutButton=$('#analysis-cut'),analysisLineCutButton=$('#analysis-line-cut'),analysisEditTargetSelect=$('#analysis-edit-target'),threeEditStatus=$('#three-edit-status'),threeEditHelp=$('#three-edit-help'),threeEditOverlay=$('#three-edit-overlay'),analysisRemoveSelected=$('#analysis-remove-selected'),analysisKeepSelected=$('#analysis-keep-selected'),analysisUndo=$('#analysis-undo'),analysisRedo=$('#analysis-redo'),analysisResetEdit=$('#analysis-reset-edit'),analysisExportSelected=$('#analysis-export-selected'),analysisCutWidth=$('#analysis-cut-width'),analysisCutWidthValue=$('#analysis-cut-width-value'),analysisCutDepth=$('#analysis-cut-depth'),analysisCutDepthValue=$('#analysis-cut-depth-value');
...
function updateSectionClipPlaneWorld(){
 ...
 const local=sectionLocalPlane();if(!local)return;
 sceneState.obj.updateMatrixWorld(true);
 sceneState.sectionClipPlane.copy(local).applyMatrix4(sceneState.obj.matrixWorld);
 if(sceneState.backend==='WEBGPU'&&sceneState.sectionClipGroup){
  sceneState.sectionClipGroup.clippingPlanes=[sceneState.sectionClipPlane];
  sceneState.sectionClipGroup.enabled=true;
 }
}
...
let analysisRegions=[],nextAnalysisRegionId=1,nextAnalysisColorIndex=0,analysisFocusedRegionId=null,analysisEditTool='select',analysisEditTargetKey=null,analysisEditTargetMode='auto',analysisCutStroke=null,analysisCutScreen=[];
...
analysisNavigateButton.onclick=()=>{analysisEditTool='select';analysisEditTargetKey=analysisEditTargetMode==='auto'?null:analysisEditTargetMode;analysisCutStroke=null;analysisCutScreen=[];updateAnalysisEditorControls();updateThreeEditUi(currentLanguage==='ja'?'通常操作':'Navigate');request3DRender()};
...
analysisEditTargetSelect.onchange=()=>{analysisEditTargetMode=analysisEditTargetSelect.value;analysisEditTargetKey=analysisEditTargetMode==='auto'?null:analysisEditTargetMode;updateAnalysisEditorControls();updateThreeEditUi();request3DRender()};
...
analysisCutWidth.oninput=()=>{analysisCutWidthValue.value=(+analysisCutWidth.value).toFixed(1)+' mm';updateThreeEditUi()};analysisCutDepth.oninput=()=>{analysisCutDepthValue.value=(+analysisCutDepth.value).toFixed(1)+' mm';updateThreeEditUi()};
...
async function start3D(){
 ...
 sceneState={scene,camera,renderer,obj:null,analysisMesh:null,backend,needsRender:true,medicalVolume:null,mprPlaneGroup:null,mprPlaneEntries:null,mprPlaneSignature:'',mprInteractionActive:false,axisWidget,sectionClipPlane,sectionClipGroup};
 ...
 const dragSectionPlane=(start,e)=>{
  ...
  updateMpr3DPlanePositions();updateSectionClipPlaneWorld();updateSectionViewUi();request3DRender();schedulePlaneRender(p);
 };
...
function cutRunsFromVoxelStroke(v,points,widthMm,depthMm){
...
async function applyCutStroke(points,key=analysisEditTargetKey){
 ...
 try{
  const st=segmentEditState[key],refs=snapshotAnalysisRegionsForSegment(key),cut=cutRunsFromVoxelStroke(v,points,+analysisCutWidth.value||.8,+analysisCutDepth.value||5);
...
function updateThreeEditUi(message=null){
 ...
 if(threeEditStatus){
  ...
  else threeEditStatus.textContent=modeLabel+' · '+targetLabel+' · '+(+analysisCutWidth.value).toFixed(1)+' mm × '+(+analysisCutDepth.value).toFixed(1)+' mm';
 }
 if(threeEditHelp){
  if(analysisEditTool==='pen')threeEditHelp.textContent=tr('editPenHint');
