...
const APP_VERSION='2026.09.23-93';const APP_BUILD='93';
...
function createCutPlacementFrame(event,canvas,camera,preferredKey=null){
 ...
 if(!anchorWorld){
  const hits=raycaster.intersectObject(obj,true).filter(h=>h.object?.isMesh&&!h.object?.userData?.cutResultPreview);
  if(hits.length)anchorWorld=hits[0].point.clone();
  else{
