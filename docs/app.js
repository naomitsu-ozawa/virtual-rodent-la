...
function createCutPlacementFrame(event,canvas,camera,preferredKey=null){
 if(!sceneState?.obj||!volume)return null;
 const rect=canvas.getBoundingClientRect(),mouse=new THREE.Vector2(((event.clientX-rect.left)/Math.max(rect.width,1))*2-1,-((event.clientY-rect.top)/Math.max(rect.height,1))*2+1),raycaster=new THREE.Raycaster();raycaster.setFromCamera(mouse,camera);
 const obj=sceneState.obj,surface=surfaceSegmentPointerVoxel(event,canvas,camera,preferredKey);if(!surface?.hit?.point)return null;
 const viewDir=new THREE.Vector3();camera.getWorldDirection(viewDir).normalize();
 const anchorWorld=surface.hit.point.clone(),inv=obj.matrixWorld.clone().invert(),toVoxelDir=vec=>{const q=vec.clone().transformDirection(inv).normalize();return{x:q.x,y:-q.y,z:q.z}};
 return{plane:new THREE.Plane().setFromNormalAndCoplanarPoint(viewDir,anchorWorld),ray:toVoxelDir(raycaster.ray.direction),right:toVoxelDir(new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0)),up:toVoxelDir(new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,1)),key:surface.key||preferredKey||null,anchor:surface};
}
