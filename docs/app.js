...
if (!hasWebGPU) {
  ...
  startDemo().catch((error) => {
    ...
    viewport.innerHTML = `
      <div class="error-panel">
        <strong>WebGPUの初期化に失敗しました。</strong>
        <span>ブラウザのWebGPU対応状況を確認してください。</span>
      </div>
    `;
  });
}
...
async function startDemo() {
  ...
  renderer.domElement.addEventListener('pointermove', (event) => {
    ...
    rodent.rotation.x = THREE.MathUtils.clamp(
      rodent.rotation.x + dy * 0.006,
      -0.55,
      0.55,
    );
  });
...
function createPlaceholderRodent() {
  ...
  const snout = new THREE.Mesh(
    new THREE.SphereGeometry(0.38, 40, 24),
    bodyMaterial,
  );
