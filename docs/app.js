...
const APP_VERSION='2026.09.22-20';const APP_BUILD='20';
...
languageToggle.onclick=()=>{applyLanguage(currentLanguage==='ja'?'en':'ja');renderAnalysisResults();updateSliceAnalysisUi();updateGpuStatus();updateRenderModeControl()};
applyLanguage('ja');;
...
let sliceAnalysisEnabled=false;
const sliceAnalysisCache={axial:null,coronal:null,sagittal:null},sliceAnalysisSelection={axial:null,coronal:null,sagittal:null};
function updateSliceAnalysisUi(){
 if(!sliceAnalysisToggle)return;
 sliceAnalysisToggle.removeAttribute('data-i18n');sliceAnalysisToggle.textContent=sliceAnalysisEnabled?tr('sliceAnalysisOff'):tr('sliceAnalysis');sliceAnalysisToggle.classList.toggle('is-active',sliceAnalysisEnabled);
 sliceAnalysisResult?.classList.toggle('is-hidden',!sliceAnalysisEnabled);
 if(sliceAnalysisEnabled&&sliceAnalysisReadout&&!Object.values(sliceAnalysisSelection).some(Boolean))sliceAnalysisReadout.textContent=tr('sliceAnalysisHint');
}
function sliceAnalysisPixelSpacing(p){
 if(!volume)return[1,1];const[sx,sy,sz]=volume.spacing;
 return p==='axial'?[sx,sy]:p==='coronal'?[sx,sz]:[sy,sz];
}
function cacheSliceAnalysisPlane(p,idx,dims,values){
 if(!values)return;sliceAnalysisCache[p]={idx,width:dims[0],height:dims[1],values:values instanceof Float32Array?values:new Float32Array(values)};
}
function sliceCanvasPoint(p,event){
 const c=planes[p],rect=c.canvas.getBoundingClientRect(),x=Math.max(0,Math.min(c.canvas.width-1,Math.floor((event.clientX-rect.left)/Math.max(rect.width,1)*c.canvas.width))),y=Math.max(0,Math.min(c.canvas.height-1,Math.floor((event.clientY-rect.top)/Math.max(rect.height,1)*c.canvas.height)));
 return{x,y};
}
function analyzeSliceSelection(p,a,b){
 const cache=sliceAnalysisCache[p],idx=+planes[p].slider.value;if(!cache||cache.idx!==idx)return;
 let x0=Math.min(a.x,b.x),x1=Math.max(a.x,b.x),y0=Math.min(a.y,b.y),y1=Math.max(a.y,b.y);
 const point=Math.max(x1-x0,y1-y0)<3;if(point){x0=x1=Math.round((x0+x1)/2);y0=y1=Math.round((y0+y1)/2)}
 let n=0,sum=0,min=Infinity,max=-Infinity;
 for(let y=y0;y<=y1;y++)for(let x=x0;x<=x1;x++){const v=cache.values[y*cache.width+x];if(!Number.isFinite(v))continue;n++;sum+=v;if(v<min)min=v;if(v>max)max=v}
 if(!n)return;const mean=sum/n,[psx,psy]=sliceAnalysisPixelSpacing(p),area=n*psx*psy,selection={idx,x0,x1,y0,y1,point,n,mean,min,max,area};sliceAnalysisSelection[p]=selection;
 const label=p[0].toUpperCase()+p.slice(1),fmtHu=v=>(Math.round(v*10)/10).toLocaleString(currentLanguage==='ja'?'ja-JP':'en-US');
 sliceAnalysisReadout.textContent=point?label+' · HU '+fmtHu(mean)+' · ('+x0+', '+y0+')':label+' · ROI '+n+' px · '+area.toFixed(2)+' mm² · Mean '+fmtHu(mean)+' HU · Min '+fmtHu(min)+' · Max '+fmtHu(max);
 safeRenderPlane(p);
}
function drawSliceAnalysisOverlay(p,idx,ctx){
 if(!sliceAnalysisEnabled)return;const sel=sliceAnalysisSelection[p];if(!sel||sel.idx!==idx)return;
 ctx.save();ctx.strokeStyle='#00e5ff';ctx.fillStyle='rgba(0,229,255,.13)';ctx.lineWidth=2;
 if(sel.point){ctx.beginPath();ctx.moveTo(sel.x0-8,sel.y0);ctx.lineTo(sel.x0+8,sel.y0);ctx.moveTo(sel.x0,sel.y0-8);ctx.lineTo(sel.x0,sel.y0+8);ctx.stroke()}
 else{ctx.fillRect(sel.x0,sel.y0,sel.x1-sel.x0+1,sel.y1-sel.y0+1);ctx.strokeRect(sel.x0+.5,sel.y0+.5,sel.x1-sel.x0,sel.y1-sel.y0)}
 ctx.restore();
}
...
function set3DState(mode){
 ...
 if(sliceAnalysisToggle)sliceAnalysisToggle.disabled=!volume;
...
sliceAnalysisToggle.onclick=()=>{
 if(!volume)return;sliceAnalysisEnabled=!sliceAnalysisEnabled;
 if(sliceAnalysisEnabled&&volumeAnalysisMode){volumeAnalysisMode=false;volumeAnalysisToggle.textContent=tr('volumeMode');volumeAnalysisToggle.classList.remove('is-active');volumeAnalysisResult.classList.add('is-hidden');clearAnalysisHighlight()}
 updateSliceAnalysisUi();renderAll();
};
volumeAnalysisToggle.onclick=async()=>{
 if(!volume)return;
 if(!volumeAnalysisMode){
  if(sliceAnalysisEnabled){sliceAnalysisEnabled=false;updateSliceAnalysisUi();renderAll()}
...
async function renderPlane(p){
 ...
 const img=ctx.createImageData(...dims),analysisValues=new Float32Array(dims[0]*dims[1]),low=+wc.value-(+ww.value)/2,scale=255/Math.max(+ww.value,1);let q=0;
 ...
 for(let y=0;y<dims[1];y++)for(let x=0;x<dims[0];x++){
  ...
  analysisValues[y*dims[0]+x]=v;const g=Math.max(0,Math.min(255,Math.round((v-low)*scale)));let rr=g,gg=g,bb=g;
 ...
 cacheSliceAnalysisPlane(p,idx,dims,analysisValues);ctx.putImageData(img,0,0);drawAnalysisOverlay(p,idx,ctx);drawSliceAnalysisOverlay(p,idx,ctx);refreshMpr3DPlaneTexture(p)
}
function paintSourcePlane(c,dims,values,p='axial',idx=0){
 cacheSliceAnalysisPlane(p,idx,dims,values);const ctx=c.canvas.getContext('2d');c.canvas.width=dims[0];c.canvas.height=dims[1];
 ...
 ctx.putImageData(img,0,0);drawAnalysisOverlay(p,idx,ctx);drawSliceAnalysisOverlay(p,idx,ctx);refreshMpr3DPlaneTexture(p);
}
...
function ensureMpr3DPlanes(){
 ...
 const defs={
  ...
  sagittal:{canvas:planes.sagittal.canvas,color:0xf3cc30,css:'#f3cc30',size:[pz*scale,py*scale],rotation:[0,Math.PI/2,0],label:'SAGITTAL'}
 };
 const entries={};
 for(const [key,def] of Object.entries(defs)){
  const root=new THREE.Group();root.name='mpr_plane_'+key;root.rotation.set(...def.rotation);
  ...
  const geometry=new THREE.PlaneGeometry(def.size[0],def.size[1]),material=new THREE.MeshBasicMaterial({map:texture,transparent:true,opacity:.66,side:THREE.DoubleSide,depthWrite:false});
  ...
  group.add(root);entries[key]={root,texture,mesh,border,label};
 }
...
function refreshMpr3DPlaneTexture(p){
 const entry=sceneState?.mprPlaneEntries?.[p];if(entry){entry.texture.needsUpdate=true;request3DRender()}
}
...
async function start3D(){
 ...
 viewport.appendChild(renderer.domElement);sceneState={scene,camera,renderer,obj:null,analysisMesh:null,backend,needsRender:true,medicalVolume:null,mprPlaneGroup:null,mprPlaneEntries:null,mprPlaneSignature:''};
 ...
 const begin3DInteraction=()=>{if(threeRenderMode==='surface')set3DPixelRatio(interactive3DPixelRatio)};
 const end3DInteraction=()=>{if(threeRenderMode==='surface')set3DPixelRatio(full3DPixelRatio)};
