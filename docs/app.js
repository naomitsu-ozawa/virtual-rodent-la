...
const mpr3DSliceSliders=Object.fromEntries(['axial','coronal','sagittal'].map(p=>[p,$('#mpr3d-slider-'+p)]));
function syncMpr3DSliceSliders(){
 let any=false;
 for(const[p,el]of Object.entries(mpr3DSliceSliders)){
  ...
  if(el.max!==src.max)el.max=src.max;if(el.value!==src.value)el.value=src.value;
...
for(const[p,el]of Object.entries(mpr3DSliceSliders)){
 if(!el)continue;
 el.addEventListener('input',()=>{planes[p].slider.value=el.value;planes[p].slider.dispatchEvent(new Event('input'))});
 el.addEventListener('change',()=>{planes[p].slider.value=el.value;planes[p].slider.dispatchEvent(new Event('change'))});
}
function schedulePlaneRender(p,immediate=false){
 ...
 if(!immediate&&perSliceFilteredActive()){filteredPlaneRunners[p]();return}
...
async function start3D(){
 ...
 const full3DPixelRatio=Math.min(devicePixelRatio,2);let active3DPixelRatio=full3DPixelRatio,wheelQualityTimer=null,fastKeepOverlays=false,fastInteractionActive=false,fastInteractionTier=-1;
