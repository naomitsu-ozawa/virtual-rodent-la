...
analysisCutWidth.oninput=()=>{analysisCutWidthValue.value=(+analysisCutWidth.value).toFixed(1)+' mm';updateThreeEditUi()};
...
async function applyCutStroke(points,key=analysisEditTargetKey){
 ...
 const v=current3DVolume||volume;if(!v)return;
 const st=segmentEditState[key],refs=snapshotAnalysisRegionsForSegment(key),cut=cutRunsFromVoxelStroke(v,points,+analysisCutWidth.value||.8);
...
function updateThreeEditUi(message=null){
 ...
 if(threeEditStatus){
  ...
  else threeEditStatus.textContent=modeLabel+' · '+targetLabel+' · '+(+analysisCutWidth.value).toFixed(1)+' mm';
 }
 if(threeEditHelp){
