...
export class MedicalVolumeRenderer{
 ...
 support(v){
  ...
  const bytes=s.columns*s.rows*s.slices.length*2,touch=(navigator.maxTouchPoints||0)>0;
  // On iPad/iPhone the renderer and compute paths share one GPUDevice. A multi-GB
  // 3D texture can lose that device and take the entire WebGPU renderer with it.
  // Keep full-resolution GPU volume enabled only while the allocation stays inside
  // a conservative mobile budget; source-backed MPR/surface rendering remains exact.
  const mobileBudget=512*1024*1024;
  if(touch&&bytes>mobileBudget)return{ok:false,reason:'Full-resolution GPU volume exceeds mobile safety budget ('+(bytes/1073741824).toFixed(2)+' GiB > 0.50 GiB)'};
  return{ok:true};
 }
