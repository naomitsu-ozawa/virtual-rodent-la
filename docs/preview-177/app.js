...
import { MedicalVolumeRenderer, extractSourceThresholdRuns } from './medical-volume.js?v=20260925-build177';
import { unzip } from 'https://esm.sh/fflate@0.8.2';
const APP_VERSION='2026.09.25-177';const APP_BUILD='177';
...
function shouldAutoPrepareResidentGpu(v){
 ...
 const bytes=residentGpuVolumeBytes(v);
 if(isIPhoneRuntime()||isIPadRuntime())return bytes<=512*1024*1024;
 return true;
}
...
async function prepareResidentGpuVolume(v){
 const mv=sceneState?.medicalVolume;if(!mv||!v?.sourceBacked)return false;
 const support=mv.support(v);
 if(!support.ok){
  residentMprReadbackDisabled=true;
  if(/safety budget/i.test(support.reason||'')){
   setGpuComputeBackend(gpuFilterRuntime.device?'WEBGPU READY':'CPU',support.reason);
   footer.textContent=(currentLanguage==='ja'?'iPadではこのCTのフル解像度GPUボリューム（'+fmt(residentGpuVolumeBytes(v))+'）を安全のため無効化しました。サーフェス/MPRはフル解像度で利用できます。':'Full-resolution GPU volume is disabled on this mobile device for this '+fmt(residentGpuVolumeBytes(v))+' CT. Surface/MPR remain full resolution.');
  }
  return false;
 }
 if(!shouldAutoPrepareResidentGpu(v))return false;
