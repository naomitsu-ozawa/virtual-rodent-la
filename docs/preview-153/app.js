...
const APP_VERSION='2026.09.24-153';const APP_BUILD='153';
...
function syncMpr3DOverlayPresentation(){
 ...
 for(const key of ['axial','coronal','sagittal']){
  ...
  entry.mesh.material.opacity=volumeMode?mpr3DVolumeOpacity:mpr3DSurfaceOpacity;
...
function setMpr3DOverlayVisible(key,visible){
 ...
 if(key==='axes'){if(sceneState?.axisWidget)sceneState.axisWidget.visible=!!visible}
  ...
  const entry=sceneState?.mprPlaneEntries?.[key];if(entry){if(visible)refreshMpr3DPlaneTexture(key);syncMpr3DOverlayPresentation()}
 }
...
function ensureMpr3DPlanes(){
 ...
 for(const [key,def] of Object.entries(defs)){
  ...
  root.visible=!!mpr3DVisibility[key];mesh.visible=!!mpr3DVisibility[key];mesh.material.opacity=threeRenderMode==='volume'?mpr3DVolumeOpacity:mpr3DSurfaceOpacity;border.visible=!!mpr3DVisibility[key];label.visible=!!mpr3DVisibility[key];
