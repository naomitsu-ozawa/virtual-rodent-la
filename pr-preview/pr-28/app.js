...
folderInput.onchange=async e=>{
 ...
 const isProject=f=>/\.vrlab$/i.test(f.name);
 const projects=all.filter(isProject).sort((a,b)=>b.lastModified-a.lastModified),files=all.filter(f=>!isProject(f));
 let chosen=null;
 if(projects.length){
  try{pendingProject.value=unpackProject(new Uint8Array(await projects[0].arrayBuffer()));chosen=projects[0]}
  catch(err){console.warn('Project in folder could not be read.',err)}
 }
 ...
 const ja=currentLanguage==='ja',others=projects.length>1?(ja?'（'+projects.length+'件中、最新を使用）':' (newest of '+projects.length+')'):'';
...
async function selectSeries(s){
 ...
 try{
  ...
  projectSaveBtn.disabled=false;void applyPendingProject();
 }catch(e){
...
function saveProject(){
 ...
 try{
  ...
  downloadBlob(new Blob([bytes],{type:'application/zip'}),base+'_'+new Date().toISOString().slice(0,10)+PROJECT_EXTENSION);
  footer.textContent=(currentLanguage==='ja'?'プロジェクトを保存しました · '+fmt(bytes.byteLength)+' · DICOMフォルダに入れておくと、フォルダを開くだけで自動適用されます':'Project saved · '+fmt(bytes.byteLength)+' · keep it in the DICOM folder to apply it automatically when the folder is opened');
 }catch(e){console.error(e);footer.textContent=(currentLanguage==='ja'?'プロジェクトを保存できませんでした: ':'Could not save project: ')+String(e.message||e)}
}
